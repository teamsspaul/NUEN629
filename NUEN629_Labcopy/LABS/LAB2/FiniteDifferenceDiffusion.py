...
# The code to the 1-group, fixed-source, steady-state, finite difference diffusion equation will work
# in the following way.  We will order our unknowns $ijk$ by spanning the $i$ coordinates first, then 
#the $j$ coordinates, and finally the $k$ coordinates.  As an illustration we start at $111$, $211$, 
#$311$, $\dots$ $I11$, then go to $121$, $221$, $\dots$, $I21$, and so on until we get to $1J1$, $2J1$,
# $\dots$ $IJ1$, and then go to $112$, $212$, $312$, \dots $I12$.  This means that we first span a single
# $xy$ plane at a time before increasing the $z$ coordinate. It will be useful to have a function to
# translate between $ijk$ coordinates and a single coordinate $l$:
...
x,y,z,phig = steady_multigroup_diffusion(G,(I,J,K),(Nx,Nx,1),BCGs,Sigmatg,Sigmasgg,nuSigmafg,nug,chig,D,Q,maxits=2)
...
x,y,z,phig = steady_multigroup_diffusion(2,(I,J,K),(Nx,Nx,1),BCGs,Sigmaag,Sigmasgg,nuSigmafg,nug,chig,D,Q)
...
def time_dependent_mg_diffusion(phi0, v, dt, T, G,Dims,Lengths,BCGs,
                                Sigmatg,Sigmasgg,nuSigmafg,nug,chig,D,Q,lintol=1.0e-8,grouptol=1.0e-6,maxits = 12):
    """Solve a time dependent diffusion problem
    Inputs:
    phi0:       initial condition
    v:          particle speed
    ...
    for step in range(numsteps):
        ...
        print("================================")
        print("Solving for time",(step+1)*dt)
        x,y,z,phi0 = steady_multigroup_diffusion(G,Dims,Lengths,BCGs,Sighat,Sigmasgg,nuSigmafg,nug,chig,D,Qhat)
    return x,y,z,phi0
...
v[:,:,:,1] = 0.1
phi0 = v*0
x,y,z,phig = time_dependent_mg_diffusion(phi0, v, 0.1, 1.0, G,(I,J,K),(Nx,Nx,1),BCGs,
...
def inner_iteration(G,Dims,Lengths,BCGs,Sigmar,Sigmasgg,
                    D,Q,lintol=1.0e-8,grouptol=1.0e-6,maxits = 20,LOUD=False):
    I = Dims[0]
    J = Dims[1]
    K = Dims[2]
    iteration = 1
...
def kproblem_mg_diffusion(G,Dims,Lengths,BCGs,Sigmarg,Sigmasgg,nuSigmafg,
    ...
    while not(converged):
        ...
        x,y,z,phi0 = inner_iteration(G,Dims,Lengths,BCGs,Sigmarg,Sigmasgg,D,Qhat)
        ...
        converged = (((np.abs(knew-k) < tol) and 
                     (np.linalg.norm(np.reshape(phi0,I*J*K*G))-solnorm)/solnorm < tol) 
                    or (iteration > maxits))
        k = knew
        phi0 /= k
        phiold = phi0.copy()
...
x,y,z,k,iterations,phig = kproblem_mg_diffusion(G,(I,J,K),(Nx,Nx,Nx),BCGs,Sigmarg,Sigmasgg,nuSigmafg,
                          chig,D,lintol=1.0e-13,grouptol=1.0e-13,tol=1.0e-12,maxits = 5, k = 1, LOUD=False)
...
x,y,z,k,iterations,phig = kproblem_mg_diffusion(G,(I,J,K),(Nx,Nx,Nx),BCGs,Sigmarg,Sigmasgg,nuSigmafg,
                          chig,D,lintol=1.0e-8,grouptol=1.0e-6,tol=1.0e-8,maxits = 15, k = 1, LOUD=True)
print("k =",k,"Number of iterations =",iterations)
plt.plot(x,phig[:,0,0,0], label="group 1")
plt.show()
plt.pcolor(x,y,phig[:,:,0,0])
...
#This one should have alpha be 2.5
I = 10
J = 10
K = 1
Nx = 10
...
x,y,z,alpha,iteration,phig = alpha_mg_diffusion(G,(I,J,K),(Nx,Nx,Nx),BCGs,Sigmarg,Sigmasgg,nuSigmafg,
                                            chig,D,v,
                                            lintol=1.0e-10,grouptol=1.0e-10,tol=1.0e-6,maxits = 20, alpha = 1.5, LOUD=True)
...
#This one should have alpha be -1
I = 10
J = 10
K = 1
Nx = 10
...
#This one should have alpha be 1.25
I = 4
J = 4
K = 1
Nx = 10
...
x,y,z,alpha,iteration,phig = alpha_mg_diffusion(G,(I,J,K),(Nx,Nx,Nx),BCGs,Sigmarg,Sigmasgg,nuSigmafg,
                                            chig,D,v,
                                            lintol=1.0e-10,grouptol=1.0e-10,tol=1.0e-6,maxits = 20, alpha = 1.251, LOUD=True)
print("alpha =",alpha,"Number of iterations =",iteration)
plt.plot(x,phig[:,0,0,0], label="group 1")
plt.show()
plt.pcolor(x,y,phig[:,:,0,0])

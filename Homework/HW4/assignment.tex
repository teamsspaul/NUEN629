...
\begin{homeworkProblem}[Homework 4 Problem Background]
  ...
  Where the source was divided by $4\pi$ because it was assumed to be
  isotropic. The final simplification for our problem will be in space.
  ...
  \begin{equation*}
    ...
    \right)\psi(z,\mu)=
    \Sigma_s2\pi\int_{-1}^1d\mu'\
    K(\mu_0)\psi(z,\mu')
    +\frac{q}{4\pi}.
  \end{equation*}

  Where $\Sigma_s$ was moved outside the integral because it has
  no angular dependence, $\mu_0$ is the cosine of the scattering angle
  (representing the probability of scattering into $\mu$),
  and integration over the azimuthal angle occured because $K(\mu_0)$ is
  assumed to be uniform and not depend on that angle. This means:
  \begin{equation*}
    K(\mu_0)=\frac{\text{Total events leading to $\mu$}}
    {\text{Total scattering events possible}}=\frac{1}{2}
  \end{equation*}
  This assumes that scattering is isotropic, and that I understand what
  is going on. 

  
  Now we employ some Legendre
  (pronounced Legendary...just kidding) polynomial expansion

...
\begin{homeworkProblem}[Homework 4 Problem Background]
  ...
  \begin{equation*}
    ...
    \right)\psi(z,\mu)=
    \Sigma_s\int_{4\pi}d\hat{\Omega}'\
    K(\hat{\Omega}'\cdot\hat{\Omega})\psi(z,\mu)
    +\frac{q}{4\pi}.
  \end{equation*}

  Where $\Sigma_s$ was moved outside the integral for the following part,
  where we employ some Legendre

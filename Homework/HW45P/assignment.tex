...
\begin{homeworkProblem}[Homework 5 Background]
  ...
  Also both plus and minus terms were written here, but the first equation
  in this solution method only uses the positive terms. This is because
  using the negative $\beta$'s yields the same real part as the positive
  $\beta$'s, with opposite complex parts (the complex cancels).
  Thats why the $\sum$ only goes to $N/2$ and the solution is multiplied
  by $2$.
